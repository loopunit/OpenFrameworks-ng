...
void ofSystemAlertDialog(string errorMessage){
		...
		NSRunInformationalAlertPanel(@"Alert", (NSString *)msgStrRef, @"Ok", nil, nil);
		[pool drain];
	#endif
...
ofFileDialogResult ofSystemLoadDialog(string windowTitle, bool bFolderSelection, string defaultPath){
	...
	CFStringRef msgStrRef = CFStringCreateWithCString(NULL, defaultPath.c_str(), kCFStringEncodingUTF8);
	NSURL *url = [NSURL URLWithString:(NSString*)msgStrRef];
	[open setDirectoryURL:url];
	...
	if (result == NSOKButton)
	{
		...
		results.filePath = [selectedFile UTF8String];
	}
...
/*	NSOpenPanel *open = [NSOpenPanel openPanel];
	...
	CFStringRef msgStrRef = CFStringCreateWithCString(NULL, defaultName.c_str(), kCFStringEncodingUTF8);
	NSURL *url = [NSURL URLWithString:(NSString*)msgStrRef];
	[open setDirectoryURL:url];
